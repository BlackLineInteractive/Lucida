...
static std::string ReadShader(const std::string &rel_path) {
  ...
  for (const auto &prefix : {base, base + "../", base + "../../", base + "../../../", base + "../Resources/", std::string("")}) {
...
public:
  ...
  void Render(const FrameTime &time) override {
    ...
    @autoreleasepool {
      ...
      SetVec3(m_uniforms.sun_direction, glm::length(m_environment.sun_direction) > 1e-4f ? glm::normalize(m_environment.sun_direction) : Vec3(0.527f, 0.422f, 0.738f));
      m_uniforms.sun_intensity = m_environment.sun_intensity;
      SetVec3(m_uniforms.sun_color, m_environment.sun_color);
      m_uniforms.sun_enabled = m_environment.sun_enabled ? 1 : 0;
      m_uniforms.water_height = m_environment.water_height;
      m_uniforms.water_speed = m_environment.water_speed;
      m_uniforms.water_frequency = m_environment.water_frequency;
      m_uniforms.water_foam = m_environment.water_foam;
      m_uniforms.exposure = m_environment.exposure;
      m_uniforms.tonemap_mode = m_environment.tonemap_mode;
      m_uniforms.gamma = m_environment.gamma;
      m_uniforms.dither_strength = m_environment.dither_strength;
      m_uniforms.ao_radius = m_environment.ao_radius;
      m_uniforms.ao_intensity = m_environment.ao_intensity;
      m_uniforms.ao_samples = m_environment.ao_samples;
